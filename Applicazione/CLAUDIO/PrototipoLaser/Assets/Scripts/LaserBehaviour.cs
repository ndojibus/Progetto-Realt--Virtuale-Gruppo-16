...
public class LaserBehaviour : MonoBehaviour {
    ...
    private void RayCast() {
        ...
        if (Physics.Raycast(this.transform.position, this.transform.forward, out hit))  //se raggio che va dalla posizione dell'oggetto verso avanti colpisce qualcosa
        {
            ...
            if (hit.collider.tag == "Activator")                                        //se l'oggetto colpito dal raggio ha la tag Activator
                hit.collider.gameObject.SendMessageUpwards("Activate");                 //instanzia la funzione Activate sull'interruttore
        }
        else
